...
class FeatureReducer:
    def __init__(self, stock_dict, n_components_stock, n_components_news):
        ...
        self.pca = pca.PCA()    # Only Feature reducing algorithm used currently is PCA, but more could be added below 
    ...
    def use_pca(self):
        ...
        Args:
            stock_dict: Dictionary with stock symbols as keys and a nested dictionary containing
        ...
        for stock, data in self.stock_dict.items():
            # Extract stock and news data
            stock_data = data['stock_data']
            news_data = data['news_data']

            # Ensure no missing values by filling or interpolating
            ...
            self.pca.fit(stock_data_scaled)
            stock_data_pca = self.pca.transform(stock_data_scaled, self.n_components_stock)
            ...
            self.pca.fit(news_data_scaled)
            news_data_pca = self.pca.transform(news_data_scaled, self.n_components_news)
    ...
    Returns:
        self.stock_data: Dictionary with dimension reduced stock data 
    """
